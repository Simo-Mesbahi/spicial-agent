...
## Diagnosing a rejected live evaluation

Both `eval:generation` and `eval:grounding` retain `providerAttempts` in each result and print failed attempts directly in their CLI summary. Each attempt records the actual provider and redacted/bounded model, HTTP status, normalized reason, allowlisted error code/parameter, latency and token usage. Missing usage remains null. No raw response, upstream message, request headers, key or customer data is included. Old reports cannot recover discarded transport details; update and make at most one new evaluation call when diagnosis requires it.

Gemini errors may contain a numeric `error.code`, a canonical `error.status`, and a single-error array. The runtime now reads these shapes while retaining only known status names. OpenAI billing/quota codes and transient rate-limit codes are retained separately when supplied. Unknown codes remain null. `upstream_rejected` alone is not proof of an invalid key, quota, model or payload; inspect the HTTP status and retained code before changing configuration. A 404/`NOT_FOUND` indicates an unavailable resource and still requires checking the actual model/endpoint and access. No retries, extra provider calls, payload changes or guardrail relaxation are introduced by diagnostics.

The current Gemini compatibility transport follows Google's documented `/v1beta/openai/chat/completions` endpoint. Google documents `reasoning_effort=none` for the configured 2.5 Flash family: https://ai.google.dev/gemini-api/docs/openai . Provider availability for a particular account must still be demonstrated live. OpenAI error-code reference: https://developers.openai.com/api/docs/guides/error-codes .
