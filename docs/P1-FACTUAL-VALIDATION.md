...
4. Require exactly one ordered verdict per sentence. For Gemini, the provider judges only `index/kind/verdict/issues` and receives the compact JSON contract in the prompt instead of a provider-side `json_schema`. Provenance is server-owned: the validator binds each factual sentence to the draft's already-validated `evidenceRefs`, reconstructs canonical citations from the current Evidence Pack and re-runs the full strict Zod/reference/quote validation before assessment. Missing, repeated or extra indices, inconsistent verdicts, invalid JSON, unknown draft references or invalid reconstructed evidence cause abstention. A supported factual sentence must still have server-verified evidence; a referenced business sentence cannot be relabeled as courtesy. This reduces provider schema complexity without granting the model any authority over provenance.
