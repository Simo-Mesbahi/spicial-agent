...
3. Reserve one atomic D1 quota slot for the organization. Run one independent completion with no tools, no retries, at most 1,200 completion tokens and four seconds, further bounded by evidence/session expiry.
4. Require exactly one ordered verdict per sentence. Missing, repeated or extra indices, inconsistent verdicts, unknown references, invented quotes and invalid JSON cause abstention. A supported factual sentence needs a citation; a referenced business sentence cannot be relabeled as courtesy. Scalar quotes must match canonical JSON exactly; documentary quotes must appear in content, never just a title. These checks authenticate citations, not entailment.
