...
For the Gemini free-tier qualification baseline, completion calls are paced start-to-start with a 7500 ms minimum interval. The live qualification workflow now defaults to `gemini-3.5-flash-lite`; earlier approved Gemini model identifiers remain allowlisted for explicit compatibility runs. The pacing helper itself never retries. Structured understanding and natural generation keep strict `json_schema` output; factual validation deliberately uses prompt-constrained JSON because hosted qualification repeatedly proved that Gemini's OpenAI-compatible endpoint rejects the judge schema with HTTP 400 even after schema projection. Factual outputs still pass strict local Zod validation, and provenance is reconstructed from server-owned draft evidence references rather than provider-supplied citations.

The hosted runs on 2026-09-23 measured repeated deadline exhaustion at the former 20s/5s/4s ceilings. The preproduction qualification profile therefore uses bounded server-owned deadlines of 45 seconds for structured provider requests, 12 seconds for natural generation and 12 seconds for factual validation. These values do not change normal production defaults when the overrides are unset or extend evidence/session expiry. The structured evaluator now forwards the provider-timeout override explicitly; this closes the observed gap where the workflow advertised 30 seconds while the synthetic structured runtime still used the 20-second default.

The same live evidence isolated a Gemini compatibility rejection specific to factual validation. The provider therefore returns only the semantic verdict contract; citations and evidence identity remain server-owned and are reconstructed from the draft's validated `evidenceRefs` before the original strict Zod/reference/quote checks run. The first governed grounding scenario is used as a factual transport smoke before structured/generation spend. The grounding evaluator also fails fast after a systemic transport signature (auth/configuration or three consecutive request rejections), and later grounding shards are skipped. A partial run still fails the automated gate; these optimizations only prevent unnecessary provider calls.

The runner caps the qualification budget before executing: 100 required structured completions, at most 30 additional structured completion calls for up to six clean-scenario retries after recoverable `503/timeout` failures, 10 generation completions, 70 grounding completions and 20 embedding calls. The total completion ceiling is therefore 210. Retries are qualification-only, scenario-level and observable. A provider fallback aborts the current multi-turn attempt immediately; a recoverable transport failure restarts that scenario from a fresh synthetic database so later turns are never scored against contaminated conversation state. HTTP 429 is deliberately **not** retried: the first rate-limited structured turn is classified as `provider_rate_limited` and the campaign stops fail-closed. This avoids misclassifying a provider-side rate limit as a recoverable timeout and prevents the retry allowance from being consumed. Semantic mismatches and HTTP 400 request rejections are likewise never retried. Missing metrics, exhausted retry budgets and malformed verdicts never count as passing.
