...
For the Gemini free-tier qualification baseline, completion calls are paced start-to-start with a 6500 ms minimum interval. Pacing is deterministic and qualification-only; it does **not** retry a failed provider call and therefore does not silently exceed the governed call budget. Gemini structured understanding, generation and factual validation use strict `json_schema` output for this qualification baseline. Before transport, the schema is projected onto Gemini's documented structured-output JSON Schema subset; unsupported provider keywords such as string-length constraints are omitted only from the remote schema while the complete local Zod schema remains authoritative after receipt.

The hosted runs on 2026-09-23 measured repeated deadline exhaustion at the former 20s/5s/4s ceilings. The preproduction qualification profile therefore uses bounded server-owned deadlines of 30 seconds for the provider request, 12 seconds for natural generation and 12 seconds for factual validation. These values do not change normal production defaults when the overrides are unset, do not extend evidence/session expiry, and do not add retries or provider calls.

The same live evidence isolated a Gemini compatibility rejection specific to the deeper factual-validation schema. The provider-facing factual transport therefore uses parallel citation reference/quote arrays, while the server reconstructs the canonical citation objects and performs the original strict Zod/reference/quote checks before any verdict can influence release. The grounding evaluator also fails fast after a systemic transport signature (auth/configuration or three consecutive request rejections), and later grounding shards are skipped. A partial run still fails the automated gate; the optimization only prevents unnecessary provider calls.
