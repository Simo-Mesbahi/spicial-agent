...
Run #12 then passed retrieval, the factual smoke and all 100 structured turns at 100% required accuracy, but isolated a different multilingual failure: `refund-policy-de` declared `language=de` while its actual prose was French. The generation schema is now request-scoped for language as well as evidence: its `language` enum contains only the server-requested language. The system prompt names that language explicitly, states that evidence may be written in another language and must be translated semantically, and forbids exposing raw internal status codes such as `waiting_part`. A server-side language hint check rejects obvious prose/metadata disagreement before factual validation. Qualification additionally permits one bounded language-only corrective generation per affected scenario (maximum two globally); factual failures remain non-retryable.
...
Run #14 exposed a transport/lease race rather than a model-quality regression: one structured Gemini call timed out at the 60-second provider deadline while the conversation lease was also exactly 60 seconds. The provider failure was correctly classified as `network_or_timeout`, but the expired lease caused fallback persistence to fail with a secondary HTTP 503, hiding the retryable reason from the qualification runner. Structured provider calls now renew the conversation lease for the bounded provider deadline plus safety margin, renew again after provider latency, and renew before persisting a provider fallback. The evaluator also stops a scenario immediately on any non-200 API response so later turns are never scored against potentially contaminated state.
