...
The runner caps the qualification budget before executing: 100 required structured completions, at most 30 additional structured completion calls for up to six clean-scenario retries after transient `503/429/timeout` failures, 10 generation completions, 70 grounding completions and 20 embedding calls. The total completion ceiling is therefore 210. Retries are qualification-only, scenario-level, observable and never triggered by semantic mismatches or HTTP 400 request rejections. Missing metrics, exhausted retry budgets and malformed verdicts never count as passing.
