...
Run #11 validated the structured gate at 100/100 turns with every required metric at 1.0 and no retries, then isolated a generation-only failure: the Arabic waiting-part candidate returned an unknown evidence alias. Generation structured output is now request-scoped: the `evidenceRefs` JSON-Schema item is an enum built from the exact server-owned reference keys in the current Evidence Pack. This prevents the provider from emitting translated, renamed or invented evidence identifiers while retaining the existing local reference, duplicate and factual-validation checks.

A subsequent correctness audit tightened the qualification boundary before the next live run. Every generated candidate is now passed through the same factual validator before it is eligible for human review; the generation gate requires a `supported_candidate` verdict with zero issues. Grounding negatives no longer pass merely because they were blocked: each negative must be rejected specifically as `unsupported_claim` and must include its independently authored expected issue category. Output hygiene also rejects URLs, HTML/markdown and HTML entities such as `&uuml;` before a draft can qualify.
...
The runner caps the qualification budget before executing: 100 required structured completions plus at most 30 structured retry calls, 10 generation completions plus at most 3 generation retries, 10 factual-validation completions for those exact generated candidates plus at most 3 validation retries, and 70 grounding completions plus at most 9 grounding retries. The total completion ceiling is therefore 235. The corpus-index preflight requires at most 1 embedding batch plus 1 bounded retry; retrieval then requires 20 embedding queries and permits at most 4 retry embeddings. The complete P1.7 embedding ceiling is therefore 26, and every indexing/retrieval call is accounted for before completion spend.

Retries are qualification-only, explicit and observable. Completion retries apply **only** to `network_or_timeout` and `upstream_unavailable`; semantic mismatches, invalid structured output, auth/configuration failures and HTTP 400/422 are never retried. Completion HTTP 429 remains fail-closed immediately as `provider_rate_limited`. Embedding preflights are the sole rate-limit exception: after the real run #10 exposed a single embedding 429, corpus indexing and retrieval may each perform only their explicitly budgeted paced/cooldown retry for an embedding 429; a repeated 429 stops retrieval fail-closed before completion spend. Exhausted retry budgets become an explicit systemic transport failure and cannot count as qualification success.
