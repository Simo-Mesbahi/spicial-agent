...
declare
  ...
  v_clean_query text := left(trim(coalesce(p_query,'')),500);
  v_strict tsquery;
  v_relaxed tsquery;
  v_lexemes text[];
  v_relaxed_text text;
begin
  ...
  if char_length(v_clean_query) < 2 then return; end if;

  v_strict := websearch_to_tsquery('french', v_clean_query);
  v_lexemes := tsvector_to_array(to_tsvector('french', v_clean_query));
  ...
  with scored as (
    select
      ...
      (
        ...
        + 8.0 * extensions.similarity(lower(d.title), lower(v_clean_query))
      )::real as rank
    ...
    where d.organization_id=p_organization_id
      ...
      and (
        ...
        or extensions.similarity(lower(d.title), lower(v_clean_query)) >= 0.16
      )
  ),
  deduped as (
    select scored.*,
