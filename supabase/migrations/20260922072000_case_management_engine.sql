...
begin
  ...
  loop
    ...
    v_reference := format('%s-%s-%s',v_prefix,v_year,lpad(v_value::text,6,'0'));
    exit when not exists(
      select 1 from public.service_cases
      where organization_id=p_organization_id and upper(reference)=upper(v_reference)
    );
  end loop;
...
declare
  ...
  result jsonb;
begin
  if not app_private.case_service_read_allowed(p_organization_id,(
    select service_type from public.service_cases
    where id=p_case_id and organization_id=p_organization_id
  )) then
    raise exception using errcode='42501',message='admin_access_denied';
  end if;
  if p_note is null or char_length(trim(p_note)) not between 3 and 4000
     or p_visible is null or p_version is null
     or p_request_id is null or p_request_id !~ '^[a-zA-Z0-9-]{16,80}
returns jsonb
language plpgsql
stable
...
commit;
 then
  ...
  fingerprint:=encode(extensions.digest(
    concat_ws('|',auth.uid()::text,p_case_id::text,p_visible::text,trim(p_note)),
    'sha256'
  ),'hex');
  ...
  if previous.id is not null then
    if previous.action<>'admin.note'
       or previous.response->>'fingerprint' is distinct from fingerprint then
      raise exception using errcode='40001',message='request_conflict';
    end if;
  ...
  insert into public.case_events(
    ...
    p_organization_id,p_case_id,c.status,
    case when p_visible then 'Message de votre conseiller' else 'Note interne' end,
    jsonb_build_object('detail',trim(p_note)),
    p_visible,'admin',auth.uid()
  )
  returning id into event_id;

  update public.service_cases
  set version=version+1,source_system='admin_ui',source_updated_at=now()
  where id=p_case_id;

  insert into public.audit_events(
    organization_id,actor_user_id,action,entity_type,entity_id,request_id,metadata
  ) values (
    p_organization_id,auth.uid(),'case.note','service_case',p_case_id,p_request_id,
  ...
  insert into public.case_commands(
    ...
    p_organization_id,p_case_id,p_request_id,'admin.note','completed',
    result||jsonb_build_object('fingerprint',fingerprint)
  );
  return result;
end;
$;
revoke all on function app_private.admin_add_note(uuid,uuid,integer,text,boolean,text)
  from public,anon,authenticated;
grant execute on function app_private.admin_add_note(uuid,uuid,integer,text,boolean,text)
  to authenticated;
...
as $
  select app_private.admin_add_note(
    p_organization_id,p_case_id,p_version,p_note,p_visible,p_request_id
  );
$;
revoke all on function public.admin_add_note(uuid,uuid,integer,text,boolean,text)
  from public,anon;
grant execute on function public.admin_add_note(uuid,uuid,integer,text,boolean,text)
  to authenticated;
...
declare
  ...
  c public.service_cases%rowtype;
begin
  if p_status is null or p_status not in ('assigned','resolved')
  ...
  if h.case_id is not null then
    ...
    if not app_private.case_service_read_allowed(p_organization_id,c.service_type) then
      raise exception using errcode='42501',message='admin_access_denied';
    end if;
  ...
  set status=p_status,assigned_to=coalesce(h.assigned_to,auth.uid())
  where id=h.id;

  insert into public.audit_events(
    organization_id,actor_user_id,action,entity_type,entity_id,metadata
  ) values (
    p_organization_id,auth.uid(),'handoff.'||p_status,'handoff',h.id,
...
$;
revoke all on function app_private.admin_manage_handoff(uuid,uuid,text,timestamptz)
  from public,anon,authenticated;
grant execute on function app_private.admin_manage_handoff(uuid,uuid,text,timestamptz)
  to authenticated;
...
as $
  select app_private.admin_manage_handoff(
    p_organization_id,p_handoff_id,p_status,p_expected_updated_at
  );
$;
revoke all on function public.admin_manage_handoff(uuid,uuid,text,timestamptz)
  from public,anon;
grant execute on function public.admin_manage_handoff(uuid,uuid,text,timestamptz)
  to authenticated;
