...
export function environmentLabel(env: { APP_ENVIRONMENT?: string }, url: string) {
  if (['LOCAL', 'PREPRODUCTION', 'PRODUCTION'].includes(env.APP_ENVIRONMENT ?? ''))
    return env.APP_ENVIRONMENT!;
  return ['localhost', '127.0.0.1', '[::1]', 'terminal.local'].includes(new URL(url).hostname)
    ? 'LOCAL'
    : 'NON CONFIGURÉ';
}
...
export function defaults(env: RuntimeEnv): RuntimeConfig {
  const provider = runtimeConfigSchema.shape.provider.parse(env.LLM_PROVIDER ?? 'demo');
  return {
    provider,
    ...
    dailyLimit: Math.max(
      ...
      Math.min(
        10000,
        Math.floor(
          Number.isFinite(Number(env.LLM_DAILY_LIMIT ?? 100))
            ? Number(env.LLM_DAILY_LIMIT ?? 100)
            : 100,
        ),
      ),
    ),
...
export function validateConfig(env: RuntimeEnv, config: RuntimeConfig) {
  // Admin controls may select only providers explicitly enabled by the server.
  // They still cannot change spending policy, secrets or outbound URLs.
...
export async function readSettings(db: Database, scope: string) {
  return db
    .prepare(
      'SELECT revision,config,actor,created_at FROM runtime_settings WHERE scope=? ORDER BY revision DESC LIMIT 1',
    )
    .bind(scope)
    .first<Revision>();
}
...
function providerCandidate(env: RuntimeEnv, provider: ProviderId) {
  ...
  if (provider === 'ollama')
    return {
      provider,
      model: configuredModel(env, provider),
      label: `Ollama · ${configuredModel(env, provider)}`,
    };
  if (provider === 'compatible')
    return {
      ...
      model: configuredModel(env, provider),
      label: configuredModel(env, provider)
        ? `Compatible · ${configuredModel(env, provider)}`
        : 'Compatible · modèle à configurer',
    };
  return null;
}

export function availableProviders(env: RuntimeEnv) {
  let allowed: ProviderId[];
  try {
    allowed = enabledProviders(env);
  } catch (error) {
    return [
      {
        provider: 'demo',
        model: '',
        label: 'Réponses documentaires',
        available: true,
        reason: null,
      },
      {
        ...
        model: '',
        label: 'Configuration fournisseurs',
        available: false,
        reason: error instanceof Error ? error.message : 'Configuration IA invalide.',
      },
    ];
  }

  const candidates: { provider: string; model: string; label: string }[] = [
    { provider: 'demo', model: '', label: 'Réponses documentaires' },
  ];
  ...
  return candidates.map((item) => {
    const state = publicModelConfig({
      ...env,
      LLM_PROVIDER: item.provider,
      LLM_MODEL: item.model || undefined,
    });
